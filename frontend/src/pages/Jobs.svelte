<script>
    ...
    import {jwt_token, user, isAuthenticated, myFreelancerId} from "../store";
    ...
    function isUserAdmin() {
        return isAuthenticated && $user && $user.user_roles && $user.user_roles.includes("admin");
    }
    ...
    function assignToFreelancer(jobId) {
        const job = jobs.find((j) => j.id === jobId);
        const freelancer = freelancers.find((f) => f.id === job.updatedFreelancerId);
        var config = {
            method: "put",
    ...
    function completeJob(jobId) {
        var config = {
            method: "put",
            url: api_root + "/api/service/completeJob",
            headers: { Authorization: "Bearer " + $jwt_token },
            data: {
...
<div class="row mb-3">
    <div class="col-auto">
        <label for="filter-earnings" class="col-form-label">Earnings: </label>
    </div>
    <div class="col-3">
        <input
            id="filter-earnings"
    ...
    <div class="col-auto">
        <label for="filter-type" class="col-form-label">Job Type: </label>
    </div>
    <div class="col-3">
        <select bind:value={jobType} class="form-select" id="filter-type">
...
<table class="table">
    ...
    {#each jobs as job}
        <tr>
            ...
            <td>{job.jobState}</td>
            <td>
                {#if isUserAdmin() && job.freelancerId === null}
                    <div class="d-flex justify-content-end">
                        <select bind:value={job.updatedFreelancerId} class="form-select" id="assignedFreelancer">
                            <option value=""></option>
                            {#each freelancers as freelancer}
                                <option value="{freelancer.id}">{freelancer.name}</option>
                            {/each}
                        </select>
                        {#if job.updatedFreelancerId !== ""}
                            <button
                                on:click={() => assignToFreelancer(job.id)}
                                type="submit"
                ...
                {:else}
                    {job.freelancerId ? job.freelancerId : ''}
                {/if}
            </td>
            <td>
                ...
                {/if}
                {#if job.jobState !== "DONE" && job.freelancerId && job.freelancerId === $myFreelancerId}
                    <button
                        type="button"
                        class="btn btn-primary btn-sm"
                        on:click={() => { completeJob(job.id); }}>
                        Complete Job
                    </button>
                {/if}
            </td>
